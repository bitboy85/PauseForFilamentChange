...
# reworked by bitboy
# https://community.ultimaker.com/topic/30611-ender-3-pro-silent-board-pause-at-height/?tab=comments#comment-253697
...
class PauseForFilamentChange(Script):
    def __init__(self) -> None:
        super().__init__()

    def getSettingDataString(self) -> str:
        return """{
            "name": "Pause for Filament change",
            "key": "PauseForFilamentChange",
            "metadata": {},
            "version": 2,
            "settings":
            {
                ...
                {
                    "label": "Retraction",
                    "description": "How much filament must be retracted before pause.",
                    "unit": "mm",
                    "type": "float",
                    "default_value": 300
                },
                ...
                "creality_workaround":
                {
                    "label": "Enable Creality workaround",
                    "description": "Mainly for Creality printers to lock z axis and disable Extruder for easy Filament change.",
                    "type": "bool",
                    "default_value": true
                },
                "home_z":
                {
                    "label": "Re-home z axis",
                    "description": "Home z-axis again if accidentally moved. Make sure that the machine will not hit the print!",
                    "type": "bool",
                    "default_value": false
                }
            }
    ...
    def execute(self, data: List[str]) -> List[str]:
        ...
        creality_workaround = self.getSettingValueByKey("creality_workaround")
        home_z = self.getSettingValueByKey("home_z")
        ...
        for index, layer in enumerate(data):
            ...
            for line in lines:
                ...
                prepend_gcode += ";script: PauseForFilamentChange.py\n"
                if pause_at == "height":
                ...
                if creality_workaround:
                    prepend_gcode += self.putValue(M = 17) + " ; enable stepper to prevent moving\n"
                    prepend_gcode += self.putValue(M = 18) + " X ; Creality workaraound enable z, disable extruder stepper\n"
                    prepend_gcode += self.putValue(M = 18, S = 999) + " ; disable stepper timeout\n"
                else:
                    prepend_gcode += self.putValue(M = 17) + " X Y Z ; enable stepper to prevent moving\n"
                    prepend_gcode += self.putValue(M = 18) + " E ; disable extruder stepper\n"
                    prepend_gcode += self.putValue(M = 18, S = 0) + " ; disable stepper timeout\n"
